...
export default function Explorateur() {
  ...
  useEffect(() => {
    ...
    axios.get('/api/historique').catch(() => ({ data: [] })).then(histRes => {
      if (histRes) {
                  ...
                  <div className={styles.vide}>
                    ...
                    <div className={styles.tableCount}>
                      ...
                      <button className={styles.btnMajLot}
                        ...
                        {lotMajLoading
                          ? <><Loader size={13} className={styles.spin}/> Mise à jour en cours...</>
                          : <><RefreshCw size={13}/> Tout à mettre à jour</>}
                      </button>
                    </div>
                                  ...
                                  montreMisAJour
                                    ...
                                    : <button className={styles.btnMaj}
                                        ...
                                        {ajoutsEnCours[`update_${p.reference}`] === 'loading'
                                          ? <><Loader size={12} className={styles.spin}/> Mise à jour...</>
                                          : <><RefreshCw size={12}/> à mettre à jour</>}
                                      </button>
                                )}
                                {p._dans_excel === false && (
                                  ajoutEtat === 'done'
                                    ...
                                    : <button className={styles.btnAjouter}
                                        onClick={() => ajouterDansExcel(p)}
                                        disabled={ajoutEtat === 'loading'}>
                                        {ajoutEtat === 'loading'
                                          ? <Loader size={12} className={styles.spin}/>
                                          : <PlusCircle size={12}/>}
                                        {ajoutEtat === 'loading' ? 'Ajout en cours...' : 'à ajouter'}
                                      </button>
                                )}
